...
class BlackjackEnv(Env):
    ...
    def __init__(self, config):
        ...
        if self.share_policy and self.num_players>1:
            ...
            self.state_shape = [[2 + 10 + self.num_players -1] for _ in range(self.num_players)]  # 2->3
        else:
    ...
    def _extract_state(self, state):
        ...
        for i in range(len(player_cards)):
            player_score.append(get_score(player_cards[i]))
        # print('player',player_score,player_cards)
        dealer_score = get_score(dealer_cards)


        # obs = np.array([my_score, dealer_score, remained_card_value])
        player_score = [item/31 for item in player_score]
        obs = np.array([my_score/31]+player_score+[dealer_score/31] + [card/8 for card in card_distribution])
    ...
    def get_payoffs(self):
        ...
        for i in range(self.num_players):
            if i == 0:
                ...
                for item in self.game.winner['player' + str(i)]:
                    if item == 2:
                        payoffs[i].append(1)  # Dealer bust or player get higher score than dealer
                    elif item == 1:
                        payoffs[i].append(0)  # Dealer and player tie
