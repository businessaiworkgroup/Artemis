...
class Env(object):
    ...
    def reset(self):
        ...
        self.action_recorder = []
        return [self._extract_state(state[the_player_id]) for the_player_id in range(self.num_players)]

    def step(self, actions, raw_action=False):
        ''' Step forward
        ...
        if not raw_action:
            # print(actions)
            actions = [self._decode_action(action) for action in actions]

        self.timestep += 1
        # Record the action for human interface
        for the_player_id in range(self.num_players):
            self.action_recorder.append((the_player_id, actions[the_player_id]))
        next_state = self.game.step(actions)
        # print('next', next_state[0])
        # print('next', next_state[1])
        return_next_state = [self._extract_state(next_state[the_player_id]) for the_player_id in range(self.num_players)]
    ...
    def run(self, is_training=False):
        ...
        while not self.is_over():
            ...
            for player_id in range(self.num_players):
                if not is_training:
                    # print('player0',state[0])
                    # print('player1',state[1])
                    action, _ = self.agents[player_id].eval_step(state[player_id])
                else:
                    action = self.agents[player_id].step(state[player_id])
        ...
        # print(len(trajectories),len(trajectories[0]), len(trajectories[0][0]), len(trajectories[0][0][0]))
        # print(trajectories[0])

        # print('0',trajectories[0][0])
        # print('1',trajectories[0][1])
