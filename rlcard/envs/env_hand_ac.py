...
class Env(object):
    ...
    def get_actions(self, is_training=False):
        actions = []
        action_probabililties = []
        for player_id in range(self.num_players):
            state = self.get_state(player_id)
            if not is_training:
                if self.share_policy:
                    action, action_probabililty = self.agents[0].eval_step(state)
                else:
                    action, action_probabililty = self.agents[player_id].eval_step(state)
            else:
                if self.share_policy:
                    action, action_probabililty = self.agents[0].step(state)
                else:
                    action, action_probabililty = self.agents[player_id].step(state)

            actions.append(action)
            action_probabililties.append(action_probabililty)
        return actions, action_probabililties
    ...
    def run(self, is_training=False):
        ...
        if self.action_phase == 0:
            ...
            # print(is_training)
            agents_actions, agent_action_probabililties = self.get_actions(is_training=is_training)
            ...
            action_probabililty = agent_action_probabililties[player_id]
        ...
        while not self.is_over():
            ...
            # Save action
            trajectories[player_id].append(action_probabililty)
            ...
            if self.action_phase == 0:
                # agents_actions = [2 for _ in range(self.num_players)]
                agents_actions, agents_action_probabililties = self.get_actions(is_training=is_training)
                if self.num_players > 1:
                    player_id = self.game.decide_prior(agents_actions)
                else:
                    player_id = 0
